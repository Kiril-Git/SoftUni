...
--		6
     ...
     SELECT `deposit_group`, SUM(`deposit_amount`) AS 'total_sum'
FROM
    `wizzard_deposits` WHERE magic_wand_creator = 'Ollivander family'
...
SELECT left(first_name, 1) AS 'first_letter' FROM wizzard_deposits  WHERE 
deposit_group = 'Troll Chest' GROUP BY first_letter ORDER BY first_letter;
...
AS SELECT employee_id, department_id, salary, manager_id 
FROM employees
WHERE salary > 30000;
